...
WHERE 
    DATE_FORMAT(url.modified_time, '%Y-%m-%d') BETWEEN '2024-01-01' AND '2024-03-04';

"""
df_DATA = pd.read_sql_query(sql_query,mydb)
df_DATA
df_DATA['Rank_P5_Call1'] = df_DATA[(df_DATA['leadStatusnew'] == "L5 - Pitched") & (df_DATA['LeadSubStatus'] == "Call1")].groupby(['Mobile','leadStatusnew','LeadSubStatus'])['ModifiedDate'].rank("dense", ascending=True)
...
print(df_DATA)
df_DATA.to_csv('CRM_LEADS.csv',index = False)
...
spreadsheetId = '1sTXYN_j7sJyy-WtPqcifJTEWNrNPixaI5P5jOISkW3s'
sh = gc.open_by_key(spreadsheetId)

# Define the sheet name and CSV file path
sheetName = 'RAW'
csvFile = 'CRM_LEADS.csv'
